...
class UIDebugger:
    ...
    def draw_text_overlay(self, frame: np.ndarray, gesture: str, 
        ...
        lightning_main = (0, 255, 255)  # vàng tươi
        lightning_glow = (0, 200, 255)  # vàng-cyan nhạt
        fire_red = (0, 0, 255)
        fire_orange = (0, 128, 255)
        ...
        # Hiệu ứng cyberpunk tinh tế (shimmer + holo ring + particle)
        self._draw_cyber_effects(frame, box_tl, box_br, laser_cyan, laser_magenta, laser_blue)
        
        return frame

    def _draw_cyber_effects(self, frame: np.ndarray, box_tl: Tuple[int, int], box_br: Tuple[int, int],
                            c1: Tuple[int, int, int], c2: Tuple[int, int, int], c3: Tuple[int, int, int]) -> None:
        """
        Hiệu ứng cyberpunk tinh tế: halo + holo ring + particle nhẹ quanh panel.
        """
        x1, y1 = box_tl
        x2, y2 = box_br
        overlay = frame.copy()
        
        # Halo nhẹ phía sau panel
        cv2.rectangle(overlay, (x1 - 12, y1 - 12), (x2 + 12, y2 + 12), c1, -1)
        cv2.addWeighted(overlay, 0.08, frame, 0.92, 0, frame)
        
        # Holo ring (ellipse) quanh panel
        cx = (x1 + x2) // 2
        cy = (y1 + y2) // 2
        rx = (x2 - x1) // 2 + 40
        ry = (y2 - y1) // 2 + 20
        ring_overlay = frame.copy()
        cv2.ellipse(ring_overlay, (cx, cy), (rx, ry), 0, 0, 360, c2, 2, lineType=cv2.LINE_AA)
        cv2.ellipse(ring_overlay, (cx, cy), (rx + 14, ry + 8), 0, 0, 360, c3, 1, lineType=cv2.LINE_AA)
        cv2.addWeighted(ring_overlay, 0.25, frame, 0.75, 0, frame)
        
        # Particle nhẹ quanh panel
        particles = 12
        p_overlay = frame.copy()
        for _ in range(particles):
            px = random.randint(x1 - 50, x2 + 50)
            py = random.randint(y1 - 30, y2 + 30)
            r = random.randint(1, 3)
            color = random.choice([c1, c2, c3])
            cv2.circle(p_overlay, (px, py), r, color, -1, lineType=cv2.LINE_AA)
        cv2.addWeighted(p_overlay, 0.35, frame, 0.65, 0, frame)
...
if __name__ == "__main__":
    # Dùng camera thật (webcam). Đổi index 0 -> 1 nếu nhiều camera.
    ...
    debugger = UIDebugger("Debug Window")
    print("Opening webcam... Close window (X button) to exit.")
    ...
    while True:
        ...
        # Giữ tỷ lệ 16:9 cho khung camera và lật gương (mirror)
        frame = cv2.resize(frame, (1280, 720), interpolation=cv2.INTER_LINEAR)
        frame = cv2.flip(frame, 1)
        
        # Gọi debugger (landmarks=None nếu không có)
        debugger.show_debug_window(frame, test_gesture, test_angles, landmarks=None)
        
        # Xử lý sự kiện và kiểm tra trạng thái cửa sổ
        cv2.waitKey(1)
        if cv2.getWindowProperty(debugger.window_name, cv2.WND_PROP_VISIBLE) < 1:
            break
        if cv2.getWindowProperty(debugger.window_name, cv2.WND_PROP_AUTOSIZE) < 0:
            break
